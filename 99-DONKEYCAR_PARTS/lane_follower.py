...
class HandCodedLaneFollower(object):
    ...
    def detect_follow_lanes(self, org_frame, edges):
        lane_lines, frame = detect_lane_from_edges(org_frame, edges)
        final_frame = self.steer(frame, lane_lines)

        return final_frame

    def follow_lane(self, frame):  # A to Z (frame -> lane detected)
...
def detect_lane(rgb_image): 
    ...
    hsv_image = convert_rgb2hsv(rgb_image)
    edges = detect_edges(hsv_image)
    show_image('edges', edges)

    lane_lines, lane_lines_image = detect_lane_from_edges(rgb_image, edges)
    return lane_lines, lane_lines_image


def detect_lane_from_edges(frame, edges):
...
def detect_edges(hsv_image, color_code = 'g'):
    if color_code == 'y':
        ...
        lower = np.array([23, 41, 133])
    elif color_code == 'g':
        upper = np.array([71, 255, 255])
        lower = np.array([44, 54, 63])
    elif color_code == 'r':
    ...
    show_image("(%s) mask" %(color_code), mask)
...
def compute_steering_angle(frame, lane_lines):
    ...
    #print("angle_to_mid_radian, angle_to_mid_deg,steering_angle=>",angle_to_mid_radian, angle_to_mid_deg, steering_angle)
...
def test_video(video_file):
    ...
    try:
        ...
        while cap.isOpened():
            ...
            show_image("Road with Lane line", combo_image)
