...
class Mini_HBridge_DC_Motor_PWM(object):
    ...
    def run(self, speed):
        ...
        #print("Mini-HBridge-PWN: speed, throttle->",self.speed, self.throttle)
...
class RPi_GPIO_Servo(object):
    ...
    default: 2.5=min, 12.5=max
    '''
    def __init__(self, pin, freq = 50, min=2.5, max=8.5,verbose=False):
        import RPi.GPIO as GPIO
        self.pin = pin
        GPIO.setmode(GPIO.BOARD)
    ...
    def run(self, pulse):
        ...
        self.throttle = dk.utils.map_frange(pulse, 1.0, -1.0, self.min, self.max)
        if self.verbose:
            print("pulse(steering)=",pulse,",throttle=",self.throttle)
